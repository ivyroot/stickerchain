...
contract DeployStickers is Script {
    function run() external {
        ...
        BlockPlaces blockPlaces = BlockPlaces(blockPlacesContractAddress);
        require(blockPlaces.address != address(0), 'DeployStickers: BLOCK_PLACES_CONTRACT not set');
        ...
        require(stickerDesigns.owner() == address(tx.origin), 'DeployStickers: owner mismatch');
        vm.startBroadcast();
